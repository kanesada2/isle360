...
--> statement-breakpoint
CREATE INDEX `game_player_id_created_at_idx` ON `game` (`user_id`,`created_at`) WHERE used_flag = 0;--> statement-breakpoint
CREATE UNIQUE INDEX `game_player_id_daily_date_unq` ON `game` (`user_id`,`date`);--> statement-breakpoint
CREATE TABLE `score` (
	`id` text PRIMARY KEY NOT NULL,
