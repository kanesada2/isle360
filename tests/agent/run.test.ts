...
import { scoreFromToken } from '../../src/domain/replay-simulator';
...
describe('agent', () => {
  it('エージェントのスコアとリプレイトークンから復元したスコアが一致する', () => {
    ...
    const agentScore = computeScore(game);
    const token = encodeLogs(game.logs);
    const replayScore = scoreFromToken(token);
...
function printRunResult(seed: number, game: Game, score: ScoreBreakdown, replayScore: ScoreBreakdown): void {
  console.log(`\n=== Agent Run: seed=${seed} ===`);
  console.log("Plots:");
  ...
  console.log(`Score (agent):  ${score.total}`);
